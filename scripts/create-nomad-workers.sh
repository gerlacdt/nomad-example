...
# create nomad workers (ubuntu 16.04 eu-west-1)
aws ec2 run-instances --iam-instance-profile Arn=$INSTANCE_PROFILE --image-id $AMI_ID \
    --count $COUNT_WORKERS --instance-type $INSTANCE_TYPE_WORKERS --key-name $SSH_KEYNAME \
    --security-group-ids $(echo -n $SECURITY_GROUP_ID) \
    --subnet-id $(echo -n $SUBNET_ID) --user-data file://client-install.sh > output-workers.json \
    --block-device-mappings file://block-device-mapping.json

# create tags for ec2-instances
INSTANCE_IDS=$(cat output-workers.json | jq ".Instances[].InstanceId" | tr -d '"' | paste -sd " " -)
aws ec2 create-tags --resources $(echo -n $INSTANCE_IDS) --tags Key=Name,Value=nomad-worker-dev
